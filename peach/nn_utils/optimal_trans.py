...
def trace(x):
    ...
    trace = torch.diagonal(x, dim1=-2, dim2=-1).sum(dim=-1, keepdim=False)
    return trace
...
def ipot(C, x_len, x_pad, y_len, y_pad, joint_pad, beta, iteration, k):
    ...
    for _ in range(iteration):
        ...
        for _ in range(k):
            delta = 1 / (y_len * Q.matmul(sigma.view(b, m, 1)).view(b, 1, n) + y_mask)
...
def compute_itm_wpa(pl_module, batch):
    ...
    with torch.cuda.amp.autocast(enabled=False):
        ...
        for i, _len in enumerate(txt_mask.sum(dim=1)):  # mask out [sep] appended in text
            txt_mask[i, _len - 1] = False
        txt_mask[:, 0] = False  # mask out CLS in text
        img_mask[:, 0] = False  # mask out CLS in image
        if "deit" in pl_module.hparams.config["vit"]:
            img_mask[:, 1] = False
        txt_pad, img_pad = ~txt_mask, ~img_mask

        cost = cost_matrix_cosine(txt_emb.float(), img_emb.float())
        joint_pad = txt_pad.unsqueeze(-1) | img_pad.unsqueeze(-2)  # pad tokens
